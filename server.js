...
app.post("/api/quizzes", checkAuth, async (req, res) => {
  ...
  try {
    const newQuiz = await pool.query("INSERT INTO quizzes (question, options, correct_answer, image_base64, image_mimetype) VALUES ($1, $2, $3, $4, $5) RETURNING id", [question, options, correct_answer, image_base64, image_mimetype]);
...
app.put("/api/quizzes/:id", checkAuth, async (req, res) => {
  ...
  const { question, options, correct_answer, image_base64, image_mimetype } = req.body;
  try {
    const updatedQuiz = await pool.query("UPDATE quizzes SET question = $1, options = $2, correct_answer = $3, image_base64 = $4, image_mimetype = $5 WHERE id = $6 RETURNING id", [
      question,
      options,
      correct_answer,
      image_base64,
...
app.get("/api/last-submission", async (req, res) => {
  ...
  try {
    const submissionRes = await pool.query("SELECT * FROM submissions WHERE student_absen = $1 AND student_class = $2 ORDER BY submitted_at DESC LIMIT 1", [absen, kelas]);
...
app.post("/api/check-absen", async (req, res) => {
  ...
  try {
    const { rows } = await pool.query("SELECT id FROM submissions WHERE student_absen = $1 AND student_class = $2", [student_absen, student_class]);
    if (rows.length > 0) {
      return res
        .status(409)
        .json({ exists: true, message: `Nilai untuk absen ${student_absen} di kelas ${student_class} sudah ada di sistem kamu hanya bisa menyelesaikan quiz 1 kali! Silahkan hubungi guru jika ingin melakukan review/latihan soal` });
    }
...
app.post("/api/session/block", async (req, res) => {
  const { student_absen, student_class } = req.body;
  try {
    await pool.query("UPDATE quiz_sessions SET status = 'blocked', last_updated = NOW() WHERE student_absen = $1 AND student_class = $2", [student_absen, student_class]);
    res.status(200).json({ success: true });
  } catch (err) {
...
app.post("/api/session/unblock", checkAuth, async (req, res) => {
  const { student_absen, student_class } = req.body;
  try {
    await pool.query("UPDATE quiz_sessions SET status = 'active', last_updated = NOW() WHERE student_absen = $1 AND student_class = $2", [student_absen, student_class]);
...
app.get("/api/session/status", async (req, res) => {
  const { absen, kelas } = req.query;
  try {
    const { rows } = await pool.query("SELECT status FROM quiz_sessions WHERE student_absen = $1 AND student_class = $2", [absen, kelas]);
    if (rows.length === 0) {
